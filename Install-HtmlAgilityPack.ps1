...
function Install-HtmlAgilityPack { 
    ...
    $DownloadedAssemblyDir = "{0}\lib" -f $TmpPath

    Write-Verbose "Copying `"$DownloadedAssemblyDir`" to `"$Path`""
    $InstalledLib = Copy-Item "$DownloadedAssemblyDir" "$Path" -Force -ErrorAction Stop -Recurse -Passthru
...
$InstallLocation = "$PSScriptRoot\libs"
